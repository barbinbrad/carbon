...
CREATE TYPE search_entity AS enum ('Resource', 'Customer', 'Supplier', 'Job', 'Part', 'Purchase Order', 'Sales Order', 'Document');
...
BEGIN
  ...
  VALUES (employee, 'Resource', new.id, '/x/resources/' || new.id);
  RETURN new;
END;
...
BEGIN
  ...
  IF (old."fullName" <> new."fullName") THEN
    UPDATE public.search SET name = new."fullName", link = '/x/resources/people/' || new.id
    WHERE entity = 'Resource' AND uuid = new.id;
  END IF;
  RETURN new;
END;
...
CREATE POLICY "Employees with people_view can search for people" ON "search"
  FOR SELECT
  USING (coalesce(get_my_claim('people_view')::boolean, false) = true AND entity = 'Resource');
