...
import type { ExternalLink, ListElement, Image, Tag } from "../types";
...
const wrapLink = (editor: Editor, link: ExternalLink) => {
  const { selection } = editor;
  if (!selection) {
    return;
  }

  if (isElementActive(editor, ElementType.ExternalLink)) {
    unwrapLink(editor);
  }
