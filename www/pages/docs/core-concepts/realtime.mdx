...
Some data like the list of parts, customers, and suppliers are used on many pages. To avoid making a request for this data on every page, we load it once in the `<RealtimeDataProvider>` on page load. We then put the data from each table into a nanostore hook that has a getter and setter that can be accessed from any page. For example:
...
Without realtime, this data would quickly become stale. To keep data fresh, the `<RealtimeDataProvider>` will listen for updates to the table and update the nanostore hook. This means that the data will be updated on every page that uses the hook.
...
useEffect(() => {
  // fetch the whole table from the database
  fetchData();

  // listen for updates to the table and update the store
